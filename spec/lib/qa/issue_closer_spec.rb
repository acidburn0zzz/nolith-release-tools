...
describe Qa::IssueCloser do
  ...
  describe '#close_issue' do
    it 'closes the issue' do
      ...
      allow(GitlabClient)
        .to receive(:close_issue)
        .with(Project::Release::Tasks, issue)

      closer.close_issue(issue)

      expect(GitlabClient).to have_received(:create_issue_note)
      expect(GitlabClient).to have_received(:close_issue)
    end
  end
