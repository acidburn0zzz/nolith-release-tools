...
describe ReleaseTools::CommitAuthor do
  let(:git_author) { 'Your Name' }
  let(:custom_mapping) { { 'John Doe' => 'Mickael Mike' } }
  let(:custom_team) { ReleaseTools::Team.new(members: [ReleaseTools::TeamMember.new(name: 'Mickael Mike', username: 'mike')]) }

  subject { described_class.new(git_author, team: ReleaseTools::Team.new) }

  describe '#team' do
    it 'default to a Team object' do
      expect(subject.team).to be_a(ReleaseTools::Team)
    end
