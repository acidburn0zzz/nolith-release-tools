...
describe ReleaseTools::UpstreamMergeRequest do
  around do |example|
  ...
  describe '.project' do
    it { expect(described_class.project).to eq(ReleaseTools::Project::GitlabEe) }
  end
  ...
  describe '.open_mrs' do
    before do
      expect(ReleaseTools::GitlabClient).to receive(:merge_requests)
  ...
  describe '#description' do
    ...
    before do
      allow(ReleaseTools::CommitAuthor)
        .to receive(:new)
        .with('John Doe', team: an_instance_of(ReleaseTools::Team))
        .and_return(double(to_gitlab: 'John Doe'))
