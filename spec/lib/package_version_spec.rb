...
describe PackageVersion do
  ...
  describe '#edition' do
    ...
    context 'with enterprise edition packages' do
      let(:deb_amd64) { 'gitlab-ee_8.16.3-ee.1_amd64.deb' }
      let(:deb_armhf) { 'gitlab-ee_8.13.12-ee.0_armhf.deb' }
      let(:rpm_x86_64_el6) { 'gitlab-ee-8.16.3-ee.1.el6.x86_64.rpm' }
      let(:rpm_x86_64_el7) { 'gitlab-ee-8.13.11-ee.0.el7.x86_64.rpm' }
      let(:rpm_x86_64_sles13) { 'gitlab-ee-8.16.3-ee.1.sles13.x86_64.rpm' }
      let(:rpm_x86_64_sles42) { 'gitlab-ee-8.16.3-ee.0.sles42.x86_64.rpm' }

      it 'returns ee for all examples' do
        aggregate_failures 'edition' do
          expect(described_class.new(deb_amd64).edition).to eq 'ee'
          expect(described_class.new(deb_armhf).edition).to eq 'ee'
          expect(described_class.new(rpm_x86_64_el6).edition).to eq 'ee'
          expect(described_class.new(rpm_x86_64_el7).edition).to eq 'ee'
          expect(described_class.new(rpm_x86_64_sles13).edition).to eq 'ee'
          expect(described_class.new(rpm_x86_64_sles42).edition).to eq 'ee'
        end
      end
    end
  end
