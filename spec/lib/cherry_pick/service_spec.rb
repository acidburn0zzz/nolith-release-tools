...
describe CherryPick::Service do
  let(:version) { Version.new('11.4.0-rc8') }

  subject { described_class.new(Project::GitlabCe, version) }
  ...
  describe 'initialize' do
    ...
    it 'validates preparation MR' do
      stub_const('PreparationMergeRequest', spy(exists?: false))
  ...
  describe '#execute', vcr: { cassette_name: 'cherry_pick/with_prep_mr' } do
    ...
    context 'with pickable MRs' do
      def stub_picking
        ...
        allow(GitlabClient).to receive(:cherry_pick) do |_, keywords|
