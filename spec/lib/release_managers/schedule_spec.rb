...
describe ReleaseManagers::Schedule do
  let(:schedule) { described_class.new(Version.new('11.8')) }
  ...
  describe '#authorized_manager_ids' do
    it 'returns a Hash mapping release manager names to their user IDs' do
      client = instance_spy(ReleaseManagers::Client)

      allow(ReleaseManagers::Client)
        .to receive(:new)
        .and_return(client)
