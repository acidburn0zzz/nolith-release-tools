...
require 'helm/version_manager'

describe Helm::VersionManager do
  ...
  def gitlab_version(version_string)
    HelmGitlabVersion.new(version_string)
  end
  ...
  def stubbed_chart(version: '0.0.1', app_version: '0.0.1')
    instance_double(
      "ChartFile",
      version: version && HelmChartVersion.new(version),
      app_version: app_version && HelmGitlabVersion.new(app_version)
    )
  end
end
